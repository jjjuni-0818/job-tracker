...
어디 넣었는지, 지금 상태가 뭔지 한눈에 안 보였다.
연락 오면 어느 공고인지 바로 찾을 수 있어야 했다.
...
스택 고를 때 기준이 하나였다. **돈 안 드는 것.**

Supabase 무료 플랜, HuggingFace 로컬 임베딩, Groq 무료 API, GitHub Pages 배포.
서버비 0원으로 RAG까지 돌아가는 서비스를 만들 수 있다는 걸 이번에 처음 알았다.
...
RAG(Retrieval-Augmented Generation)를 파이프라인으로 직접 구현한 건 처음이다.
...
솔직히 말하면 지금 RAG는 **입문 단계**다.

현재 구현:
```
공고 텍스트 전체 → 그냥 통째로 LLM에 전달
```

진짜 RAG:
```
긴 문서 → 청킹(chunk) → 각각 임베딩
→ 질문과 유사한 chunk만 골라서 → LLM에 전달
```

공고 하나가 짧아서 지금은 통째로 넘겨도 되지만, 문서가 길어지면 청킹이 필수다.

더 깊게 들어가면 이런 개념들이 나온다.
- **청킹 전략** — 문단 단위, 문장 단위, 오버랩 방식
- **리랭킹** — 유사도 높은 것 중 진짜 관련 있는 것 재정렬
- **멀티쿼리** — 질문을 여러 방식으로 바꿔서 검색
- **하이브리드 검색** — 벡터 + 키워드 검색 조합

일단 돌아가는 걸 만들고, 깊이는 다음 단계에서 파고들 생각이다.
...
Groq는 Meta의 Llama 모델을 무료로 빠르게 제공하는 서비스다.
`llama3-8b-8192`는 구버전 모델명이고, Groq가 새 버전으로 업데이트하면서 구버전을 폐기했다.

Groq 문서 찾아보니까 현재 지원 모델로 바뀌어 있었다. 한 줄만 바꾸면 됐다.

```python
# before — 폐기된 모델
model="llama3-8b-8192"

# after — 현재 지원 모델 (llama 3.1 경량 빠른 버전)
model="llama-3.1-8b-instant"
```

`instant`는 빠른 응답용 경량 버전이라는 뜻이다. 취준 트래커 정도엔 충분하다.
...
무엇보다 RAG 파이프라인을 처음으로 직접 구현해봤다. 개념으로 알고 있던 것과 직접 짜는 건 완전히 달랐다.

공고 URL 하나 넣었을 뿐인데 AI가 그 공고 기반으로 면접 질문을 뽑아주는 걸 보고 "이게 되네?" 했다.

아직 청킹도 없고 제대로 된 RAG는 아니다. 근데 일단 돌아간다. 다음 단계는 더 깊이 파고드는 것.
